#!/usr/bin/env python3
"""GDR 페이퍼 트레이딩 — 실주문 없이 신호·가상 체결만 기록 (2026-07-15).

배경(README "메타 감사"): 지금까지 4개 규칙기반 전략은 10년치 과거
데이터 전체로만 판정했고 진짜 홀드아웃이 없었다. 이 스크립트는 앞으로
실제로 쌓일 새 데이터를 진짜 한 번도 안 본 홀드아웃으로 축적하기 위한
장치다. **실제 주문은 절대 내지 않는다** — KIS 시세 조회(읽기 전용)만
쓴다.

라벨/파라미터는 GDR 95종목 격자 실험에서 이미 확정된 챔피언(f_fill=1.0,
k_stop=1.0)을 그대로 승계 — 결과를 보고 다시 고르지 않는다.

두 단계로 나눠 하루 두 번 실행한다(장중 지속 폴링 없이도 같은 정보로
당일청산을 재현 가능 — g0_backtester의 classify_bar와 동일한 방식으로
장 마감 후 고가/저가/종가만으로 resolve):

  --mode entry   : 장 시작 직후 실행. 오늘 시가로 GDR 신호 생성 + 포지션
                   사이징 + 서킷브레이커 체크 후 로그에 미결 항목 기록.
  --mode resolve : 장 마감 후 실행. 오늘 실제 고가/저가/종가로 그날의
                   미결 항목들을 해소(target_hit/stop_hit/ambiguous/
                   close_exit)하고 로그를 갱신.

로그 위치: data/paper_trading/gdr_trades.jsonl (gitignore 처리됨 — 개인
실행 기록이지 소스코드가 아님). 하트비트: data/paper_trading/heartbeat.txt.

사용법: python scripts/paper_trade_gdr.py --mode entry|resolve [--universe default|expanded]
"""
...
F_FILL, K_STOP = 1.0, 1.0            # 95종목 격자 실험에서 확정된 챔피언 승계
...
TR_ID_INQUIRE_PRICE = "FHKST01010100"  # 주식현재가 시세
...
def main() -> None:
    ...
    if mode == "entry":
        ...
        print("사용법: python scripts/paper_trade_gdr.py --mode entry|resolve [--universe default|expanded]")
