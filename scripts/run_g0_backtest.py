#!/usr/bin/env python3
"""VCB-Gap 전략을 실제 KRX 일봉으로 G0 백테스트 (STAGE 7 항목 9·10).

pytest에는 포함하지 않는다 — 여러 종목·다년치 실제 네트워크 호출이라
시간이 걸리고 KRX 서버 상태에 좌우된다.

매일 D 이전(D-1까지)의 봉으로 vcb_gap_signal을 계산하고, D의 실제 시가를
넣어 신호를 만든다 — 미래 데이터를 보지 않는다(look-ahead 없음).

기본 동작은 사전 등록된 격자(PREREGISTERED_GRID, 목표/손절 배수 6개 조합)를
전부 돌려서 투명하게 보고한다 — 결과를 보고 나서 조합을 추가로 더 찾는
행위는 데이터마이닝이므로 하지 않는다. 네트워크 호출은 종목당 한 번만
하고(원본 일봉 캐시), 6개 조합은 그 캐시된 데이터로 신호만 다시 계산한다.

사용법: python scripts/run_g0_backtest.py [--years N] [--tickers T1,T2,...] [--single]
  --single: 격자 대신 기본 조합(1.5, 1.0) 하나만 실행.
"""
...
from phase0.strategy.vcb_gap import PREREGISTERED_GRID, vcb_gap_signal
...
def generate_signals(bars: list[OhlcvBar], k_target: float, k_stop: float) -> list:
    signals = []
    for i, b in enumerate(bars):
        history = bars[:i]   # D 이전(=D-1까지) — D 당일 데이터는 절대 안 봄
        sig = vcb_gap_signal(history, b.open, b.date, k_target=k_target, k_stop=k_stop)
        if sig is not None:
            signals.append(sig)
    return signals
...
def main() -> None:
    ...
    print(f"{len(tickers)}종목, {start}~{end} 실데이터로 VCB-Gap 전략 G0 백테스트 시작")
    ...
    header = f"{'k_target':>9}{'k_stop':>8}{'신호수':>8}{'거래일':>8}{'충돌봉%':>9}{'E_cons':>11}{'E_opt':>11}   판정"
    print(header)
    print("-" * len(header))

    for k_target, k_stop in grid:
        signals_by_ticker = {
            t: generate_signals(bars, k_target, k_stop) for t, bars in raw_bars_by_ticker.items()
        }
        verdict = run_g0_multi(daily_bars_by_ticker, signals_by_ticker, cost_base=cost_base)
        print(
            f"{k_target:>9.2f}{k_stop:>8.2f}{verdict.n_trades:>8}{verdict.n_trading_days:>8}"
